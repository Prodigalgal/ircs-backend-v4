...
class SystemConfigChangedListener {
    ...
    static final String QUEUE_NAME = "q.content.config_changed";

    private final SystemConfigRepository repository;
    private final ObjectMapper objectMapper;

    @RabbitListener(queues = QUEUE_NAME, autoStartup = "${app.content.config-listener.enabled:true}")
    void handle(String payload) {
        try {
            ...
            log.warn("Failed to parse content system config change event", ex);
        }
    }
}
