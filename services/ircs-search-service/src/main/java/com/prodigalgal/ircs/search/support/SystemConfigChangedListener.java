...
class SystemConfigChangedListener {
    ...
    static final String QUEUE_NAME = "q.search.config_changed";

    private final SystemConfigRepository repository;
    private final ObjectMapper objectMapper;

    @RabbitListener(queues = QUEUE_NAME, autoStartup = "${app.search.config-listener.enabled:true}")
    void handle(String payload) {
        try {
            ...
            log.warn("Failed to parse search system config change event", ex);
        }
    }
}
