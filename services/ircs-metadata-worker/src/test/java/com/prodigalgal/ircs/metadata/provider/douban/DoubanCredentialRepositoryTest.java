...
class DoubanCredentialRepositoryTest {
    ...
    private final DoubanCredentialRepository repository = DoubanCredentialRepository.forTest(
    ...
    void fetchesOptionalCredentialFromCredentialService() {
        ...
        transport.enqueue(response(200, """
                        [{
                          ...
                          "provider":"DOUBAN",
                          "name":"dev",
                          "secretPayload":{"cookie":"dbcl2=secret","user_agent":"ua"},
                          "priority":9,
                          "rateLimit":1,
        ...
        assertThat(credential.orElseThrow().cookie()).isEqualTo("dbcl2=secret");
        OutboundHttpRequest request = transport.requests.getFirst();
        assertThat(request.uri()).isEqualTo(URI.create(
                "http://credential-service/internal/credentials/providers/DOUBAN/leases?limit=2"));
