...
class SystemConfigChangedListener {
    ...
    static final String QUEUE_NAME = "q.portal.config_changed";

    private final SystemConfigRepository repository;
    private final ObjectMapper objectMapper;

    @RabbitListener(queues = QUEUE_NAME, autoStartup = "${app.portal.config-listener.enabled:true}")
    void handle(String payload) {
        try {
            ...
            log.warn("Failed to parse portal system config change event", ex);
        }
    }
}
