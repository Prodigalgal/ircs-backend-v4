...
class SystemConfigChangedListener {
    ...
    static final String QUEUE_NAME = "q.normalization.config_changed";

    private final SystemConfigRepository repository;
    private final ObjectMapper objectMapper;

    @RabbitListener(queues = QUEUE_NAME, autoStartup = "${app.normalization.config-listener.enabled:true}")
    void handle(String payload) {
        try {
            ...
            log.warn("Failed to parse normalization system config change event", ex);
        }
    }
}
