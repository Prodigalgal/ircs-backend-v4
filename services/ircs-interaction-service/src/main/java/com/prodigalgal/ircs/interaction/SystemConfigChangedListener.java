...
class SystemConfigChangedListener {
    ...
    static final String QUEUE_NAME = "q.interaction.config_changed";

    private final SystemConfigRepository repository;
    private final ObjectMapper objectMapper;

    @RabbitListener(queues = QUEUE_NAME, autoStartup = "${app.interaction.config-listener.enabled:true}")
    void handle(String payload) {
        try {
            ...
            log.warn("Failed to parse interaction system config change event", ex);
        }
    }
}
