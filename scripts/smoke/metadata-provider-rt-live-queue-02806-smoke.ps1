param(
    ...
    [string]$CredentialBaseUrl = "http://127.0.0.1:18092",
    [int]$CredentialPort = 18092,
    [string]$ElasticsearchBaseUrl = "http://127.0.0.1:39203",
    [int]$ElasticsearchPort = 39203,
    [int]$WaitAttempts = 45,
    [int]$WaitSeconds = 2,
    [string]$Query = "The Matrix",
    [string]$Year = "1999",
    [string]$ExpectedProviderRtId = "matrix",
    [switch]$EnableRtProviderForSmoke,
...
$sourceHash = "codex02806rt$stamp"
$dataHash = "codex02806rtdata$stamp"
$rabbitPfOut = Join-Path $env:TEMP "ircs-02806-rt-live-rabbit-pf.out.log"
$rabbitPfErr = Join-Path $env:TEMP "ircs-02806-rt-live-rabbit-pf.err.log"
$credentialPfOut = Join-Path $env:TEMP "ircs-02806-rt-live-credential-pf.out.log"
$credentialPfErr = Join-Path $env:TEMP "ircs-02806-rt-live-credential-pf.err.log"
$esPfOut = Join-Path $env:TEMP "ircs-02806-rt-live-es-pf.out.log"
$esPfErr = Join-Path $env:TEMP "ircs-02806-rt-live-es-pf.err.log"
$rabbitPf = $null
$credentialPf = $null
$esPf = $null
...
function New-SmokeResult {
    ...
    [ordered]@{
        task = "028-06-I"
        slice = "rt-provider-arm64-live-rollout-queue-parity"
...
function Invoke-CurlJson {
    ...
    $tmp = Join-Path $env:TEMP ("ircs-02806-rt-live-response-" + [guid]::NewGuid() + ".json")
...
function Get-RtCredentialDiagnostic {
    $token = Get-CredentialServiceSecretToken
    $headers = @("X-IRCS-INTERNAL-TOKEN: $token")
    $leaseUrl = "$($CredentialBaseUrl.TrimEnd('/'))/internal/credentials/providers/ROTTEN_TOMATOES/leases?limit=1"
...
function Invoke-RabbitPublish {
    ...
    $requestFile = Join-Path $env:TEMP ("ircs-02806-rt-live-rabbit-publish-" + [guid]::NewGuid() + ".json")
    $responseFile = Join-Path $env:TEMP ("ircs-02806-rt-live-rabbit-response-" + [guid]::NewGuid() + ".json")
...
function Capture-MetadataEnv {
    ...
    foreach ($name in @("APP_METADATA_RT_ENABLED", "APP_METADATA_TMDB_WORKER_ENABLED", "APP_METADATA_LISTENER_ENABLED")) {
...
function Enable-MetadataRtForSmoke {
    Capture-MetadataEnv
    kubectl -n $Namespace set env deployment/ircs-metadata-worker `
        APP_METADATA_RT_ENABLED=true `
...
function Ensure-RtProviderEnabled {
    $map = Get-DeploymentEnvMap "ircs-metadata-worker"
    $providerEnabled = $map["APP_METADATA_RT_ENABLED"]
    ...
    if ($providerEnabled -eq "true" -and $workerEnabled -eq "true" -and $listenerEnabled -eq "true") {
        Capture-MetadataEnv
        return @{
            appMetadataRtEnabled = $providerEnabled
            appMetadataTmdbWorkerEnabled = $workerEnabled
            appMetadataListenerEnabled = $listenerEnabled
            patched = $false
        }
    }
    if (-not $EnableRtProviderForSmoke) {
        Finish-Smoke "SKIPPED" "RT_PROVIDER_DISABLED" @{
            appMetadataRtEnabled = $providerEnabled
            appMetadataTmdbWorkerEnabled = $workerEnabled
            appMetadataListenerEnabled = $listenerEnabled
            hint = "rerun with -EnableRtProviderForSmoke after confirming low-frequency live smoke window"
            externalRequestCount = 0
        }
    }
    Enable-MetadataRtForSmoke
    return @{
        appMetadataRtEnabled = "true"
...
function Setup-SmokeData {
    ...
    $sourceVidSql = Sql-Literal "02806-rt-$stamp"
...
function Get-EsDocument {
    ...
    $responseFile = Join-Path $env:TEMP ("ircs-02806-rt-live-es-doc-" + [guid]::NewGuid() + ".json")
    try {
...
"@
        ...
        if ($null -ne $state) {
            $parts = $state -split "\|", 4
            if ($parts[0] -eq "SUCCESS" -and $parts[1] -eq $ExpectedProviderRtId) {
                return
            }
    ...
    throw "raw metadata did not reach SUCCESS with expected rotten_tomatoes_id=$ExpectedProviderRtId; last state: $lastState"
}
...
function Wait-EsRawDocument {
    ...
    for ($attempt = 0; $attempt -lt $WaitAttempts; $attempt++) {
        ...
        if ($null -ne $doc -and $doc.found) {
            ...
            $lastState = "found enrichmentStatus='$($source.enrichmentStatus)' externalIds='$($externalIds -join ',')'"
            if ($source.enrichmentStatus -eq "SUCCESS" -and ($externalIds -contains $ExpectedProviderRtId)) {
                return
            }
        } else {
            $lastState = "missing"
    ...
    throw "ES raw document did not reach RT enrichment projection; last state: $lastState"
}
...
try {
    ...
    $credentialDiagnostic = Get-RtCredentialDiagnostic
    $gateEvidence = Ensure-RtProviderEnabled
    Mark "credential lease diagnostic completed and RT provider gate is enabled for one smoke" 1 6
    ...
    Invoke-RabbitPublish "x.process" "metadata.fetch.rt" $payload "application/json"
    $script:externalRequestIssued = $true
    Wait-QueueEmpty "q.fetch.metadata.rt"
    Mark "RT provider queue task is published and q.fetch.metadata.rt drains" 3 6

    Wait-RawMetadataSuccess
    Mark "provider result updates raw metadata to SUCCESS with rotten_tomatoes_id" 4 6

    Wait-EsRawDocument
    Mark "raw search sync refreshes ES document with RT enrichment fields" 5 6
    ...
    Finish-Smoke "SUCCESS" "RT_LIVE_QUEUE_PARITY_OK" @{
        marks = "6/6"
        query = $Query
        year = $Year
        expectedRtId = $ExpectedProviderRtId
        originalMetadataEnv = $script:originalMetadataEnv
        ...
        externalRequestMode = "metadata-worker issued one Rotten Tomatoes search GET for one queue task; script made no direct RT request"
    }
} catch {
