param(
    ...
    [string]$DoubanUrl = "https://movie.douban.com/j/subject_suggest?q=The%20Matrix",
...
$metadataConfigValues = Join-Path $repoRoot "services\ircs-metadata-worker\src\main\java\com\prodigalgal\ircs\metadata\MetadataConfigValues.java"
...
$credentialPfOut = Join-Path $env:TEMP "ircs-02806-douban-credential-pf.out.log"
$credentialPfErr = Join-Path $env:TEMP "ircs-02806-douban-credential-pf.err.log"
$credentialPf = $null
...
function Get-LocalGateEvidence {
    ...
    $configText = if (Test-Path $metadataConfigValues) { Get-Content -LiteralPath $metadataConfigValues -Raw } else { "" }
    $appYamlDefault = $null
    $appYamlMatch = [regex]::Match($applicationText, "douban-enabled:\s*\$\{APP_METADATA_DOUBAN_ENABLED:([^}]+)\}")
    ...
    return [ordered]@{
        ...
        appMetadataDoubanEnabled = Get-YamlEnvValue $manifestText "APP_METADATA_DOUBAN_ENABLED"
        ...
        applicationYamlDoubanDefault = $appYamlDefault
        metadataConfigValuesDefaultProviderEnabled = ($configText -match "DEFAULT_PROVIDER_ENABLED\s*=\s*true")
    }
}
...
function Get-KubernetesGateEvidence {
    ...
    try {
        ...
        return [ordered]@{
            status = "OK"
            appMetadataDoubanEnabled = $envMap["APP_METADATA_DOUBAN_ENABLED"]
            appMetadataRtEnabled = $envMap["APP_METADATA_RT_ENABLED"]
            appMetadataTmdbEnabled = $envMap["APP_METADATA_TMDB_ENABLED"]
...
function Get-ProviderImplementationEvidence {
    ...
    if (Test-Path $providerSourceRoot) {
        foreach ($file in Get-ChildItem -LiteralPath $providerSourceRoot -Recurse -Filter *.java) {
            ...
            if ([regex]::IsMatch($text, "implements\s+MetadataProvider") -and [regex]::IsMatch($text, "ProviderType\.DOUBAN")) {
...
function Get-CredentialCatalogEvidence {
    ...
    return [ordered]@{
        ...
        doubanSupported = ($supportedProviders -contains "DOUBAN")
...
function Invoke-CurlJson {
    ...
    $tmp = Join-Path $env:TEMP ("ircs-02806-douban-credential-response-" + [guid]::NewGuid() + ".json")
...
function Get-CredentialLeaseEvidence {
    ...
    $leaseUrl = "$credentialBaseUrl/internal/credentials/providers/DOUBAN/leases?limit=1"
...
function Classify-DoubanHttp {
    param([AllowNull()][int]$StatusCode)
    if ($null -eq $StatusCode) {
        return "DOUBAN_HTTP_STATUS_UNKNOWN"
    }
    if ($StatusCode -ge 200 -and $StatusCode -lt 300) {
        return "DOUBAN_HTTP_OK"
    }
    if ($StatusCode -eq 403) {
        return "DOUBAN_FORBIDDEN"
    }
    if ($StatusCode -eq 429) {
        return "DOUBAN_RATE_LIMITED"
    }
    if ($StatusCode -ge 500) {
        return "DOUBAN_UPSTREAM_UNAVAILABLE"
    }
    return "DOUBAN_HTTP_$StatusCode"
}

function Invoke-DoubanReadonlyHttp {
    $tmp = Join-Path $env:TEMP ("ircs-02806-douban-http-response-" + [guid]::NewGuid() + ".txt")
    $args = @(
        "-sS",
    ...
    $reason = Classify-DoubanHttp $httpStatus
    $suggestSummary = [ordered]@{
        parseStatus = "SKIPPED"
        candidateCount = 0
    }
    if ($reason -eq "DOUBAN_HTTP_OK") {
        try {
            ...
            $suggestSummary = [ordered]@{
                ...
                firstId = if ($null -ne $first) { $first.id } else { $null }
                firstTitle = if ($null -ne $first) { $first.title } else { $null }
                firstSubTitle = if ($null -ne $first) { $first.sub_title } else { $null }
                firstYear = if ($null -ne $first) { $first.year } else { $null }
                fullBodyOmitted = $true
            }
        } catch {
            $suggestSummary = [ordered]@{
                parseStatus = "FAILED"
                reason = "DOUBAN_SUGGEST_JSON_PARSE_FAILED"
                message = $_.Exception.Message
                fullBodyOmitted = $true
            }
        }
    ...
    return [ordered]@{
        status = if ($reason -eq "DOUBAN_HTTP_OK") { "OK" } else { "HTTP_FAILURE" }
        reason = $reason
        ...
        urlEffective = if ($parts.Count -ge 5) { $parts[4] } else { $DoubanUrl }
...
try {
    ...
    $doubanHttp = Invoke-DoubanReadonlyHttp

    $blockingReasons = @()
    if (-not $implementation.exists) {
        $blockingReasons += "PROVIDER_IMPLEMENTATION_MISSING"
    }
    if (-not $credentialCatalogEvidence.doubanSupported) {
        $blockingReasons += "CREDENTIAL_CATALOG_UNSUPPORTED"
    }
    if ($localGate.appMetadataDoubanEnabled -eq "false" -or $kubernetesGate.appMetadataDoubanEnabled -eq "false") {
        $blockingReasons += "DEV_PROVIDER_GATE_DISABLED"
    }
    if ($doubanHttp.status -ne "OK") {
        $blockingReasons += $doubanHttp.reason
    }
    ...
    $overallStatus = if (-not $implementation.exists) {
        "BLOCKED"
    } elseif (-not $credentialCatalogEvidence.doubanSupported) {
        "BLOCKED"
    } elseif ($doubanHttp.status -ne "OK") {
        "FAILED"
    ...
    $overallReason = if ($blockingReasons.Count -gt 0) {
        ...
        "DOUBAN_MINIMAL_LIVE_SMOKE_OK"
    }

    $resultObject = [ordered]@{
        task = "028-06-E"
        slice = "douban-provider-implementation-first-slice-smoke"
        provider = "DOUBAN"
        status = $overallStatus
        reason = $overallReason
        checkedAt = (Get-Date).ToUniversalTime().ToString("o")
        externalRequestCount = $doubanHttp.requestCount
        writeDb = $false
