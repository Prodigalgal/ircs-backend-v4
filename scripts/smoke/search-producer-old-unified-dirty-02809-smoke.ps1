param(
    ...
    [string]$ContentBaseUrl = "http://127.0.0.1:19086",
    [int]$ContentPort = 19086,
    [string]$ElasticsearchBaseUrl = "http://127.0.0.1:19206",
    [int]$ElasticsearchPort = 19206,
    [int]$WaitAttempts = 36,
    [int]$WaitSeconds = 2,
...
$oldUnifiedVideoId = [guid]::NewGuid().ToString()
$sourceHash = "codex02809dirty$stamp"
$dataHash = "codex02809dirtydata$stamp"
$oldUnifiedTitle = "Codex 02809 Dirty Unified $stamp"
$rawTitleBefore = "Codex 02809 Dirty Raw Before $stamp"
$rawTitleAfter = "Codex 02809 Dirty Raw After $stamp"
$expectedAliasTitle = "Codex 02809 Dirty Alias $stamp"
$expectedYear = "2026"
$expectedScoreText = "7.9"
$expectedScore = [double]$expectedScoreText
$expectedTmdbId = "tm$stamp"
$expectedImdbId = "tt$stamp"

$contentPfOut = Join-Path $env:TEMP "ircs-02809-dirty-content-pf.out.log"
$contentPfErr = Join-Path $env:TEMP "ircs-02809-dirty-content-pf.err.log"
$esPfOut = Join-Path $env:TEMP "ircs-02809-dirty-es-pf.out.log"
$esPfErr = Join-Path $env:TEMP "ircs-02809-dirty-es-pf.err.log"
$contentPf = $null
$esPf = $null
...
function New-SmokeResult {
    ...
    [ordered]@{
        task = "028-09"
        slice = "old-unified-dirty-lifecycle"
        status = $Status
        reason = $Reason
        checkedAt = (Get-Date).ToUniversalTime().ToString("o")
        namespace = $Namespace
        rawVideoId = $rawVideoId
        oldUnifiedVideoId = $oldUnifiedVideoId
        details = $Details
...
function Invoke-JsonRequest {
    ...
    $requestFile = Join-Path $env:TEMP ("ircs-02809-dirty-http-request-" + [guid]::NewGuid() + ".json")
    $responseFile = Join-Path $env:TEMP ("ircs-02809-dirty-http-response-" + [guid]::NewGuid() + ".json")
    try {
        if ($null -ne $Body) {
            $json = $Body | ConvertTo-Json -Depth 10 -Compress
            [System.IO.File]::WriteAllText($requestFile, $json, [System.Text.UTF8Encoding]::new($false))
        }
        $args = @("-sS", "-X", $Method, "-H", "Content-Type: application/json", "-o", $responseFile, "-w", "%{http_code}")
        if ($null -ne $Body) {
            $args += @("--data-binary", "@$requestFile")
        }
        $args += "$($ContentBaseUrl.TrimEnd('/'))$Path"
        $previous = $ErrorActionPreference
        $ErrorActionPreference = "Continue"
        ...
        if ($exit -ne 0) {
            throw "HTTP $Method $Path curl failed with exit code $exit output=$($code -join "`n")"
        }
        $status = [int]$code
        if ($ExpectedStatus -notcontains $status) {
            throw "HTTP $Method $Path returned $status body=$text"
        }
...
function Test-ContentAccess {
    $previous = $ErrorActionPreference
    $ErrorActionPreference = "Continue"
    $responseFile = Join-Path $env:TEMP ("ircs-02809-dirty-content-health-" + [guid]::NewGuid() + ".json")
    $code = & curl.exe -sS -o $responseFile -w "%{http_code}" "$($ContentBaseUrl.TrimEnd('/'))/actuator/health/readiness" 2>&1
    $exit = $LASTEXITCODE
    $ErrorActionPreference = $previous
    ...
    if ($exit -ne 0) {
        Finish-Smoke "SKIPPED" "CONTENT_SERVICE_UNAVAILABLE" @{
            curlExitCode = $exit
            output = ($code -join "`n")
        }
    }
    if ([int]$code -lt 200 -or [int]$code -ge 300) {
        Finish-Smoke "SKIPPED" "CONTENT_SERVICE_HTTP_$code" @{
            httpStatus = [int]$code
...
function Test-EsAccess {
    ...
    $responseFile = Join-Path $env:TEMP ("ircs-02809-dirty-es-root-" + [guid]::NewGuid() + ".json")
    try {
        $previous = $ErrorActionPreference
        $ErrorActionPreference = "Continue"
...
function Get-EsDocument {
    ...
    $elasticPassword = Get-ElasticPassword
    $responseFile = Join-Path $env:TEMP ("ircs-02809-dirty-es-doc-" + [guid]::NewGuid() + ".json")
    try {
        $code = & curl.exe -sS -u "elastic:$elasticPassword" `
            -o $responseFile `
            -w "%{http_code}" `
...
function Setup-SmokeData {
    $rawBeforeSql = Sql-Literal $rawTitleBefore
    $unifiedSql = Sql-Literal $oldUnifiedTitle
...
insert into raw_videos (
    ...
    title, alias_title, description, year, score,
    tmdb_id, imdb_id,
    ...
    '02809-dirty-$stamp', $sourceHashSql, $dataHashSql,
    $rawBeforeSql, $aliasSql, 'Codex 02809 dirty raw fixture', '$expectedYear', $expectedScoreText,
    $tmdbSql, $imdbSql,
    '{}'::jsonb, '[]'::jsonb,
    'PENDING', 0,
    'READY', 0,
...
insert into unified_videos (
    ...
    title, alias_title, description, year, score,
    tmdb_id, imdb_id,
    locked_fields, metadata_status
) values (
    '$oldUnifiedVideoId', now() - interval '2 days', now() - interval '2 days', 0,
    $unifiedSql, $aliasSql, 'Codex 02809 dirty unified fixture', '$expectedYear', $expectedScoreText,
    $tmdbSql, $imdbSql,
    '[]'::jsonb, 'SYNCED'
);

insert into raw_video_unified_video (raw_video_id, unified_video_id)
values ('$rawVideoId', '$oldUnifiedVideoId')
...
function Cleanup-SmokeData {
    $rawBeforeSql = Sql-Literal $rawTitleBefore
    $rawAfterSql = Sql-Literal $rawTitleAfter
    $unifiedSql = Sql-Literal $oldUnifiedTitle
    $aliasSql = Sql-Literal $expectedAliasTitle
    $tmdbSql = Sql-Literal $expectedTmdbId
    $imdbSql = Sql-Literal $expectedImdbId
...
with target_unified as (
    ...
    where id = '$oldUnifiedVideoId'
       or title in ($rawBeforeSql, $rawAfterSql, $unifiedSql)
       or alias_title = $aliasSql
       or tmdb_id = $tmdbSql
       or imdb_id = $imdbSql
...
function Assert-DbFixturePresent {
    $count = Invoke-SqlScalar "select count(*) from raw_videos rv join raw_video_unified_video rvu on rvu.raw_video_id = rv.id join unified_videos uv on uv.id = rvu.unified_video_id where rv.id = '$rawVideoId' and uv.id = '$oldUnifiedVideoId';"
    if ($count -ne "1") {
        throw "old unified dirty fixture was not seeded, count=$count"
    }
}
...
function Wait-EsUnifiedDocument {
    param(
        ...
        [string]$ExpectedTitle = $oldUnifiedTitle
    )
    $lastState = "not checked"
    for ($attempt = 0; $attempt -lt $WaitAttempts; $attempt++) {
        Start-Sleep -Seconds $WaitSeconds
        $doc = Get-EsDocument "ircs_unified_video" $oldUnifiedVideoId
        if ($null -ne $doc -and $doc.found) {
            ...
            if ($source.title -eq $ExpectedTitle `
                    -and $source.sourceCount -eq $ExpectedSourceCount `
    ...
    throw "ES unified document did not reach expected projection; expectedSourceCount=$ExpectedSourceCount last state: $lastState"
}
...
function Wait-EsRawDocument {
    ...
    for ($attempt = 0; $attempt -lt $WaitAttempts; $attempt++) {
        ...
        if ($null -ne $doc -and $doc.found) {
            $source = $doc._source
            $externalIds = ConvertTo-StringArray $source.externalIds
            $lastState = "found title='$($source.title)' aliasTitle='$($source.aliasTitle)' aggregationStatus not indexed enrichmentStatus='$($source.enrichmentStatus)' externalIds='$($externalIds -join ',')'"
            if ($source.title -eq $rawTitleAfter `
                    -and $source.aliasTitle -eq $expectedAliasTitle `
                    -and $source.enrichmentStatus -eq "PENDING" `
    ...
    throw "ES raw document did not reach updated raw projection; last state: $lastState"
}

function Assert-DirtyDbState {
    $state = Invoke-SqlScalar @"
select coalesce(rv.aggregation_status, '') || '|' ||
       coalesce(uv.metadata_status, '') || '|' ||
       (select count(*) from raw_video_unified_video where raw_video_id = '$rawVideoId' and unified_video_id = '$oldUnifiedVideoId')::text || '|' ||
       coalesce(rv.title, '')
from raw_videos rv
cross join unified_videos uv
where rv.id = '$rawVideoId' and uv.id = '$oldUnifiedVideoId';
"@
    if ($state -ne "PENDING|DIRTY|0|$rawTitleAfter") {
        throw "dirty lifecycle DB state mismatch: $state"
    }
}

function Assert-SyncedDbState {
    $state = Invoke-SqlScalar @"
select coalesce(rv.aggregation_status, '') || '|' ||
       coalesce(uv.metadata_status, '') || '|' ||
       (select count(*) from raw_video_unified_video where raw_video_id = '$rawVideoId' and unified_video_id = '$oldUnifiedVideoId')::text
...
where rv.id = '$rawVideoId' and uv.id = '$oldUnifiedVideoId';
"@
    if ($state -ne "PENDING|SYNCED|0") {
        throw "synced lifecycle DB state mismatch: $state"
    }
}
...
select
  ...
  (select count(*) from raw_video_unified_video where raw_video_id = '$rawVideoId'
      or unified_video_id in (select id from unified_videos where id = '$oldUnifiedVideoId' or title in ($rawBeforeSql, $rawAfterSql, $unifiedSql) or alias_title = $aliasSql or tmdb_id = $tmdbSql or imdb_id = $imdbSql))::text
  || '|' ||
  (select count(*) from unified_videos where id = '$oldUnifiedVideoId' or title in ($rawBeforeSql, $rawAfterSql, $unifiedSql) or alias_title = $aliasSql or tmdb_id = $tmdbSql or imdb_id = $imdbSql)::text;
"@
    if ($residue -ne "0|0|0") {
...
try {
    Test-KubectlAccess
    Test-PostgresAccess
    Assert-DeploymentReady "ircs-content-service"
    Assert-DeploymentReady "ircs-search-service"
    $contentPf = Start-PortForward "svc/ircs-content-service" $ContentPort 8080 $contentPfOut $contentPfErr
    ...
    Remove-EsDocument "ircs_unified_video" $oldUnifiedVideoId
    Setup-SmokeData
    Assert-DbFixturePresent

    Invoke-JsonRequest "POST" "/api/v1/unified-videos/batch/sync-search" @{ ids = @($oldUnifiedVideoId) } @(200) | Out-Null
    Wait-EsUnifiedDocument 1
    $total = 6
    $i = 1
    Mark "old unified fixture is seeded and initial ES unified document has sourceCount=1" $i $total; $i++

    Invoke-JsonRequest "PUT" "/api/v1/raw-videos/$rawVideoId" @{
        id = $rawVideoId
        title = $rawTitleAfter
    } @(200) | Out-Null
    Assert-DirtyDbState
    Mark "raw critical update marks old unified DIRTY and unbinds raw" $i $total; $i++

    Wait-EsRawDocument
    Mark "raw critical update publishes raw search INDEX and refreshes ircs_raw_video" $i $total; $i++

    Wait-EsUnifiedDocument 1
    Mark "old unified remains a dirty lifecycle candidate until recalculate closes it" $i $total; $i++

    Invoke-JsonRequest "POST" "/api/v1/unified-videos/$oldUnifiedVideoId/recalculate" $null @(200) | Out-Null
    Assert-SyncedDbState
    Wait-EsUnifiedDocument 0
    ...
    Wait-EsMissing "ircs_unified_video" $oldUnifiedVideoId
    Assert-NoDbResidue
    Mark "runtime search sync is clean and cleanup leaves no DB/ES fixture residue" $i $total
} catch {
    Finish-Smoke "FAILED" "OLD_UNIFIED_DIRTY_LIFECYCLE_SMOKE_FAILED" @{
        message = $_.Exception.Message
        rawVideoId = $rawVideoId
        oldUnifiedVideoId = $oldUnifiedVideoId
    }
    ...
    if ($contentPf -and -not $contentPf.HasExited) {
        Stop-Process -Id $contentPf.Id -Force -ErrorAction SilentlyContinue
    }
    if ($esPf -and -not $esPf.HasExited) {
