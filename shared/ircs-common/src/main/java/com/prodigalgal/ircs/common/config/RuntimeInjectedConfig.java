...
public final class RuntimeInjectedConfig {
    ...
    public static Optional<String> find(Environment environment, String... keys) {
        return find(environment, Arrays.asList(keys));
    }

    public static Optional<String> find(Environment environment, Iterable<String> keys) {
        if (environment == null) {
    ...
    private static String[] expandKeys(Iterable<String> keys) {
        return java.util.stream.StreamSupport.stream(keys.spliterator(), false)
                .filter(StringUtils::hasText)
